...
class TouchlineController {
    ...
    async getControllerData(request) {
        ...
        const fetchQuery = await fetch('http://' + this.controller + '/cgi-bin/ILRReadValues.cgi', {
            ...
            this.log('getControllerData: Could not fetch controller data.', error);
        });
    ...
    async applyControllerData(param, value) {
        ...
        const fetchQuery = await fetch('http://' + this.controller + '/cgi-bin/writeVal.cgi?'+queryParams, {
            ...
            this.log('applyControllerData: Could not apply data for thermostat: '+this.thermostat, error);
        });
    ...
    setTargetTemperature(value) {
        try {
            ...
            this.log('Could not set target temperature for thermostat: '+this.thermostat, error);
        }
    }
    ...
    setThermostatName(value) {
        try {
            ...
            this.log('Could not set thermostat name for thermostat: '+this.thermostat, error);
        }
    }
    ...
    setThermostatMode(mode) {
        try {
            ...
            this.log('Could not set thermostat mode for thermostat: '+this.thermostat, error);
        }
    }
    ...
    async refreshThermostatData() {
        try {
            ...
            this.log('refreshThermostatData: Could not parse data for thermostat: '+this.thermostat, error);
        }
    }
